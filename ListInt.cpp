...
void ListInt::push_back(int val)
{
	...
	{
		_tail = new Node(val, _tail); // Ќовый узел становитс€ хвостом, его указатель prev указывает на старый хвост
		_tail->_prev->_next = _tail; // ѕредыдущий узел св€зываетс€ с новым хвостом
	}
	_size++;
}
...
void ListInt::pop_back()
{
	if (empty()) return;
	Node* temp = _tail;
	_tail = _tail->_prev;                      // Ќовым хвостом становитс€ предшествующий старому хвосту узел
...
void ListInt::pop_front()
{
	if (empty()) return;
	Node* temp = _head;
	_head = _head->_next;                      // Ќовой головой становитс€ следующий после старой головы узел
...
ListInt::iterator ListInt::insert(iterator pos, int value)// возвращает позицию после вставленной
{
